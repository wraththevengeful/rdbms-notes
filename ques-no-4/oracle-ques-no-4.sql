...
CREATE TABLE EMPLOYEE (
    ...
    CONSTRAINT fk_emp_supervisor FOREIGN KEY (SENO) REFERENCES EMPLOYEE(ENO)
);
...
CREATE TABLE WORKSFOR (
    ...
    CONSTRAINT fk_wf_emp FOREIGN KEY (ENO) REFERENCES EMPLOYEE(ENO),
    CONSTRAINT fk_wf_proj FOREIGN KEY (PROJ_NO) REFERENCES PROJECT(PROJ_NO)
);
...
CREATE OR REPLACE TRIGGER increment_noe_trigger
...
CREATE OR REPLACE TRIGGER decrement_noe_trigger
...
CREATE OR REPLACE TRIGGER update_department_noe_trigger
BEFORE UPDATE OF DEPT_NO ON EMPLOYEE
FOR EACH ROW
...
BEGIN
    ...
    SET NOE = NOE + 1
    WHERE DEPT_NO = :NEW.DEPT_NO;
END update_department_noe_trigger;
/
...
CREATE OR REPLACE PROCEDURE incr(p_eno IN NUMBER, p_increment_amount IN NUMBER)
IS
BEGIN
    UPDATE EMPLOYEE
