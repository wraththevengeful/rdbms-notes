...
-- SQL query to list employees earning less than the average basic pay
...
-- SQL query to list departments with more than six employees
...
-- Create a view to keep track of department details
...
-- Trigger to increment NOE when a new employee is inserted
...
-- Trigger to decrement NOE when an employee is deleted
...
-- Trigger to update NOE when an employee is transferred
...
-- Procedure to increment the basic pay of an employee
DELIMITER //
CREATE PROCEDURE incr(IN p_eno INT, IN p_increment_amount DECIMAL)
BEGIN
    UPDATE EMPLOYEE
    SET BASIC = BASIC + p_increment_amount
    WHERE ENO = p_eno;

    -- Check for null basic pay
    IF (SELECT BASIC FROM EMPLOYEE WHERE ENO = p_eno) IS NULL THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Employee has basic pay null';
    END IF;

    -- Check for non-existent employee number
    IF (SELECT COUNT(*) FROM EMPLOYEE WHERE ENO = p_eno) = 0 THEN
        SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'No such employee number';
    END IF;
END //
DELIMITER ;
