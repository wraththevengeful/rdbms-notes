...
CREATE DATABASE IF NOT EXISTS ex3_companydb;
USE ex3_companydb;
...
BEGIN
    IF (SELECT COUNT(*) FROM WORKSFOR WHERE ENO = NEW.ENO AND DATE_WORKED = NEW.DATE_WORKED) >= 3 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Employee cannot work on more than three projects in a day';
    END IF;
END;
...
CREATE PROCEDURE INCR(IN employee_number INT, IN increment_amount DECIMAL)
BEGIN
    UPDATE EMPLOYEE
    ...
    IF ROW_COUNT() = 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'No such employee number';
    END IF;

    IF NEW.BASIC IS NULL THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Employee has basic pay null';
    END IF;
END;
