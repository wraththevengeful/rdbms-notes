...
EMPLOYEE (ENO, NAME, GENDER, DOB, DOJ, DESIGNATION, BASIC, DEPT NO, PAN, SENO)
...
d. Develop an SQL query to list details of all employees and the details of their supervisors.

e. Develop an SQL query to list the department number, department name and the number of employees in each department.

f. Develop a SQL query to list the details of employees who earn less than the average basic pay of all employees. 

g. Develop a SQL query to list the details of departments which has more than six employees working in it.

h. Create a view that will keep track of the department number, department name, number of employees in the department, and total basic pay expenditure for each department. 

i. Develop a database trigger that will not permit an employee to work on more than three projects on a day. 

j. Develop a procedure INCR that will accept employee number and increment amount as input and update the basic pay of the employee in the employee table. Include exception in the procedure that will display a message 'Employee has basic pay null if the basic pay of the employee is null and display a message 'No such employee number' if the employee number does not exist in the employee table.*/



--a. DDL to implement the schema
 create database ex3_companydb;
\u ex3_companydb;
...
CREATE OR REPLACE FUNCTION check_project_count()
RETURNS TRIGGER AS $$
BEGIN
    IF (SELECT COUNT(*) FROM WORKSFOR WHERE ENO = NEW.ENO AND DATE_WORKED = NEW.DATE_WORKED) >= 3 THEN
        RAISE EXCEPTION 'Employee cannot work on more than three projects in a day';
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER enforce_project_count
BEFORE INSERT OR UPDATE ON WORKSFOR
FOR EACH ROW EXECUTE FUNCTION check_project_count();

-- j. Procedure INCR to update the basic pay of an employee
CREATE OR REPLACE PROCEDURE INCR(employee_number INT, increment_amount DECIMAL)
AS $$
BEGIN
    UPDATE EMPLOYEE
    ...
    IF NOT FOUND THEN
        RAISE EXCEPTION 'No such employee number';
    END IF;
